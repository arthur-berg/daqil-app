...
function Calendar({
  ...
  const YEARS = React.useMemo(() => genYears(yearRange), []); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  return (
    <DayPicker
      ...
      classNames={{
        ...
        caption:
          "flex justify-center pt-1 relative items-center mb-10 w-[120px]",
        caption_label: "text-sm font-medium",
        nav: "space-x-1 flex items-center",
        nav_button_previous: cn(
          buttonVariants({ variant: "outline" }),
          "h-7 w-7 bg-transparent p-0 opacity-50 hover:opacity-100 absolute left-5 top-8"
        ),
        nav_button_next: cn(
          buttonVariants({ variant: "outline" }),
          "h-7 w-7 bg-transparent p-0 opacity-50 hover:opacity-100 absolute right-5 top-8"
        ),
        ...
        day: "h-9 w-9 text-center text-sm p-0 relative rounded-md [&:has([aria-selected].day-range-end)]:rounded-r-md [&:has([aria-selected].day-outside)]:bg-accent/50 [&:has([aria-selected])]:bg-accent first:[&:has([aria-selected])]:rounded-l-md last:[&:has([aria-selected])]:rounded-r-md focus-within:relative focus-within:z-20 hover:bg-secondary",
        day_selected:
