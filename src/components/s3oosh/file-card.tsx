  ...
  return (
    <div className="relative group flex flex-col items-center w-full">
      <div className="w-full h-16 sm:h-20 lg:h-24 flex items-center justify-center bg-gray-100 rounded-md overflow-hidden relative">
        {fileUploadProgress.status === FileStatus.Uploaded &&
          fileUploadProgress.newFileName && (
            <a
              href={getFileUrl(fileUploadProgress)}
              target="_blank"
      ...
      <p
        className="mt-1 md:mt-2 text-gray-800 text-xs sm:text-sm font-medium truncate w-full text-center"
        title={fileUploadProgress.file.name}
      >
        {fileUploadProgress.file.name.length > 15
          ? fileUploadProgress.file.name.slice(0, 15) + "..."
          : fileUploadProgress.file.name}
      </p>
      <div className="w-full h-1 mt-1 md:mt-2">
        {fileUploadProgress.status === FileStatus.Uploading &&
          !fileUploadProgress.newFileName && (
            <Progress
      ...
      {fileUploadProgress.status === FileStatus.Error && (
        <p className="text-red-500 text-xs md:text-sm mt-1 md:mt-2 text-center">
          {String(fileUploadProgress.error)}
        </p>
      )}
      <button
        onClick={(e) => {
